...
1. Pull those resources from the development environment to your local machine:
   ...
   ```shell
   grafanapi resources pull --context dev # Add `-o yaml` export resources as YAML 
   ```

1. Push the resources to production:

   ```shell
   grafanapi resources push --context prod
   ```
...
1. Ensure the current context points to the Grafana instance to backup/restore:
   ...
   ```shell
   grafanapi config use-context YOUR_CONTEXT  # for example "prod"
   ```

1. Pull all resources from your target environment:

   ```shell
   grafanapi resources pull --path ./backup-prod # Add `-o yaml` to export resources as YAML
   ```
...
1. Push the resources to restore them:
   ...
   ```shell
   grafanapi resources push --path ./backup-prod
   ```
