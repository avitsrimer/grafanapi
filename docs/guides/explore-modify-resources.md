...
```shell
grafanapi resources get dashboards --context prod | jq '.items | map({ uid: .metadata.name, datasources: .spec.panels | map(.datasource.uid)  })'
[
   {
...
```shell
grafanapi resources delete dashboards/test-dashboard-from-stg,test-dashboard-from-dev
...
```shell
grafanapi resources edit dashboard/edit-me-please
```
